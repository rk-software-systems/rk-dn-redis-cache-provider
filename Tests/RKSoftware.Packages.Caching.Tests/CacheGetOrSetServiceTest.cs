...
namespace RKSoftware.Packages.Caching.Tests
{
    ...
    public class CacheGetOrSetServiceTest
    {
        ...
        [TestMethod]
        public void TestSetCachedStream()
        {
            var source = CacheTestModel.TestModel;
            var key = CacheTestModel.TestKey;

            _cacheService.SetCachedStreamObject<CacheTestModel>(key, source, 1000, false);

            var result = _cacheService.GetCachedSteramObject<CacheTestModel>(key, false);
